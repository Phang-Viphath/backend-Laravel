...
class KHQRService
{
    ...
    /**
     * Build KHQR String per EMV QR + NBC KHQR spec.
     */
    protected function buildKHQRString(array $data): string
    {
        ...
        if (!is_string($bakongId) || trim($bakongId) === '') {
            throw new \InvalidArgumentException('Missing bakong_account_id');
        }
        ...
        $expiryMs = is_numeric($expiryMs) ? (int) $expiryMs : 0;

        // If caller provided seconds (10 digits), normalize to milliseconds.
        // Example: 1710000000 -> 1710000000000
        if ($expiryMs > 0 && $expiryMs < 1000000000000) {
            $expiryMs *= 1000;
        }

        if ($amount > 0 && $expiryMs <= $nowMs) {
            $expiryMs = $nowMs + self::QR_EXPIRY_MINUTES * 60 * 1000;
        }
